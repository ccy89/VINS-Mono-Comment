...
/**
 * @brief 计算 当前残差块对应的残差 和 Jacobian
 */
void ResidualBlockInfo::Evaluate() {
  ...
  if (loss_function) {
    ...
    sq_norm = residuals.squaredNorm();        // 获得 e^T * Σ * e
    loss_function->Evaluate(sq_norm, rho);    // 鲁棒核函数修正残差
...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info) {
  ...
  for (int i = 0; i < static_cast<int>(residual_block_info->parameter_blocks.size()); i++) {
    ...
    int size = parameter_block_sizes[i];    // 获得当前变量的维度
    parameter_block_size[reinterpret_cast<long>(addr)] = size;    // 组合 当前变量的地址 和 当前变量的维度
  }
...
void MarginalizationInfo::preMarginalize() {
  for (auto it : factors) {
    ...
    std::vector<int> block_sizes = it->cost_function->parameter_block_sizes();  // 获得当前残差块每个变量的global维度

    // 遍历当前残差块的每个变量，将变量数据放到 parameter_block_data之中
    for (int i = 0; i < static_cast<int>(block_sizes.size()); i++) {
      long addr = reinterpret_cast<long>(it->parameter_blocks[i]);    // 获得当前变量在内存的地址
...
std::vector<double *> MarginalizationInfo::getParameterBlocks(std::unordered_map<long, double *> &addr_shift) {
  ...
  sum_block_size = std::accumulate(std::begin(keep_block_size), std::end(keep_block_size), 0);  // 无用的变量
