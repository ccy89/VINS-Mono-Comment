...
class IntegrationBase {
 public:
  ...
  /**
   * @brief IMU数据初始化，初始化高斯白噪声协方差矩阵
   * 
   * @param[in] _acc_0          加速度值
   * @param[in] _gyr_0          角速度值  
   * @param[in] _linearized_ba  加速度高斯白噪声标准差
   * @param[in] _linearized_bg  角速度高斯白噪声标准差
   */
  IntegrationBase(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                  const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg)
      : acc_0{_acc_0}, gyr_0{_gyr_0}, linearized_acc{_acc_0}, linearized_gyr{_gyr_0}, linearized_ba{_linearized_ba}, linearized_bg{_linearized_bg}, 
  ...
  void midPointIntegration(double _dt,
    ...
    if (update_jacobian) {    // 更新 Jacobian 和 协方差矩阵
