...
void KeyFrame::computeBRIEFPoint() {
  ...
  if (1)
    cv::FAST(image, keypoints, fast_th, true);    // 提取新的 FAST 角点
  else {
  ...
  extractor(image, keypoints, brief_descriptors);   // 计算新提取的角点的描述子
...
bool KeyFrame::searchInAera(const BRIEF::bitset window_descriptor,
  ...
  for (int i = 0; i < (int)descriptors_old.size(); i++) {
    int dis = HammingDis(window_descriptor, descriptors_old[i]);    // 计算汉明距离
    if (dis < bestDist) {
      bestDist = dis;
      bestIndex = i;
    }
...
/**
 * @brief 匹配当前关键帧VIO中的特征 和 闭环帧新提取的特征
 * 
 * @param[out] matched_2d_old         回环帧所有匹配特征的像素坐标
 * @param[out] matched_2d_old_norm    回环帧所有匹配特征的归一化平面坐标xy
 * @param[out] status                 匹配状态，成功为1
 * @param[in] descriptors_old         回环帧所有新提取特征的描述子
 * @param[in] keypoints_old           回环帧所有新提取特征点的像素坐标
 * @param[in] keypoints_old_norm      回环帧所有新提取特征点的归一化平面坐标xy
 */
...
/**
 * @brief 通过基本矩阵检验去除匹配异常的点
 * 
 * @param[in] matched_2d_cur_norm
 * @param[in] matched_2d_old_norm
 * @param[in] status
 */
...
/**
 * @brief 利用PnP求解闭环帧的位姿（在前端VIO世界坐标系下）
 * 
 * @param[in] matched_2d_old_norm   匹配特征在闭环帧归一化平面的坐标
 * @param[in] matched_3d            匹配特征的世界坐标
 * @param[out] status               特征匹配状态，1代表成功
 * @param[out] PnP_T_old            闭环帧位姿  t_wb (前端VIO世界坐标系下)    
 * @param[out] PnP_R_old            闭环帧位姿  R_wb (前端VIO世界坐标系下)
 */
void KeyFrame::PnPRANSAC(const vector<cv::Point2f> &matched_2d_old_norm,
  ...
  cv::cv2eigen(r, R_pnp);
  R_w_c_old = R_pnp.transpose();      // R_wc

  Vector3d T_pnp, T_w_c_old;
  cv::cv2eigen(t, T_pnp);  
  T_w_c_old = R_w_c_old * (-T_pnp);   // t_wc
...
/*
  ...
  if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) {
    status.clear();
    // 当前关键帧3d点和闭环帧2d匹配 利用PnP 估计 闭环帧的位姿 T_bw （估计值，在前端VIO的世界坐标系下）
  ...
  if ((int)matched_2d_cur.size() > MIN_LOOP_NUM) {
    ...
    relative_yaw = Utility::normalizeAngle(Utility::R2ypr(origin_vio_R).x() - Utility::R2ypr(PnP_R_old).x());   // 计算偏航角

    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 20.0) {   // 相对位姿 满足阈值
      has_loop = true;              // 标记找到回环了
      ...
      // 如果 打开 fast_relocalization功能
      // 将闭环帧和当前帧的共视特征点发布出去，在VIO线程中会用到这些信息
      if (FAST_RELOCALIZATION) {
        sensor_msgs::PointCloud msg_match_points;
        msg_match_points.header.stamp = ros::Time(time_stamp);    // 这里存储的是当前关键帧的时间戳，不是闭环帧的时间戳
        // 存储共视特征点
        for (int i = 0; i < (int)matched_2d_old_norm.size(); i++) {
          // 这里的3D点的前2维是特征在闭环帧的归一化坐标，第3维是特征点的ID
        ...
        // 修改后
        Eigen::Vector3d T = PnP_T_old;
        Eigen::Matrix3d R = PnP_R_old;
        Quaterniond Q(R);
        // 存储 闭环帧的位姿 和 当前关键帧在关键帧列表中的索引
        ...
        pub_match_points.publish(msg_match_points);       // 发布闭环帧信息，用于VIO前端
      }
      return true;
    }
  }
...
/**
 * @brief 更新 闭环帧和当前关键帧之间的相对位姿信息
 * 
 * @param[in] _loop_info  新的相对位姿信息 T_loop_cur
 */
