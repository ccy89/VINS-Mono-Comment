...
class KeyFrame {
 public:
  ...
  int local_index;    // 关键帧在参与优化的关键帧序列中的索引

  Eigen::Vector3d vio_T_w_i;      // 关键帧在轨迹坐标系下的位姿 t_w1_b (闭环前的位姿) 
  Eigen::Matrix3d vio_R_w_i;      // 关键帧在轨迹坐标系下的位姿 R_w1_b (闭环前的位姿) 

  Eigen::Vector3d T_w_i;          // 关键帧闭环后的位姿 t_w0_b
  Eigen::Matrix3d R_w_i;          // 关键帧优化后的位姿 R_w0_b
  Eigen::Vector3d origin_vio_T;   // 关键帧在前端 twb
  Eigen::Matrix3d origin_vio_R;   // 关键帧原始的 Rwb
